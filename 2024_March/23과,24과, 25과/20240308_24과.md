...
- 다만 JavaScript 함수 내에서 선언한 지역 변수는 외부에서 참조하거나 변경할 수 없어서 Java의 private 접근 제한자의 특성을 나타낸다.

- 하지만 생성자 함수 안에 인스턴스 메서드를 사용한다면 자유 변수를 통해 private를 흉내 낼 수는 있지만 프로토타입 메서드를 사용한다면 자유 변수의 private 특성이 사라진다.
