...
1. 인스턴스 생성과 **`this`** 바인딩
    - **`new`** 연산자와 함께 클래스를 호출하면 **`constructor`** 의 내부 코드가 실행되기에 앞서 암묵적으로 빈 객체가 생성된다. 이 빈 객체가 바로 (아직 완성되지는 않았지만) 클래스가 생성한 인스턴스다. 이때 클래스가 생성한 인스턴스의 프로토타입으로 클래스의 **`prototype`** 프로퍼티가 가리키는 객체가 설정된다.
